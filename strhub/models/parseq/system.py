...
class PARSeq(CrossEntropySystem):
    ...
    def decode(self, tgt: torch.Tensor, memory: torch.Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        #print(tgt_query.shape) # 64, 13 ,384
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        for i, perm in enumerate(tgt_perms):
            ...
            #print(tgt_mask.shape, tgt_padding_mask.shape, query_mask.shape) #torch.Size([19, 19]) torch.Size([64, 19]) torch.Size([19, 19])
            out = self.decode(tgt_in, memory, tgt_mask, tgt_padding_mask, tgt_query_mask=query_mask)
            #print(out.shape) #1,15,384
            #print(self.head(out).shape) # 1, 8 ,37
            out = self.head(out)
            logits = out.flatten(end_dim=1)
            #print(logits.shape, tgt_out.flatten().shape) #torch.Size([64, 20, 37]) torch.Size([64, 20])
        ...
        self.log('loss', loss)
        #print(loss) #tensor(3.7929, device='cuda:7', grad_fn=<DivBackward0>)
        return loss
