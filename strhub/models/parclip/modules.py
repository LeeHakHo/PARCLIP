...
class DecoderLayer(nn.Module):
    ...
    def forward_stream(self, tgt: Tensor, tgt_norm: Tensor, tgt_kv: Tensor, memory: Tensor, tgt_mask: Optional[Tensor],
        ...
        """
        #print(tgt.shape, memory.shape) #384,512 / 384,512  #parseq에서 [32, 11(변함), 384], [32, 11, 384]
        # 원래 들어가는게 tgt_len, bsz, embed_dim인데 tgt_len이 여러개에서 한번으로 바뀜
        #print(tgt.shape, tgt_norm.shape, tgt_kv.shape)
        #torch.Size([1, 7, 384]) torch.Size([1, 7, 384]) torch.Size([6, 3, 384])
...
class Decoder(nn.Module):
    ...
    def forward(self, query, content, memory, query_mask: Optional[Tensor] = None, content_mask: Optional[Tensor] = None,
        ...
        for i, mod in enumerate(self.layers):
            ...
            #print(query.shape, content.shape) #torch.Size([64, 26, 384]) torch.Size([64, 7, 384])
